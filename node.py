...
from blockchain import Blockcahain
...
class Node:
    ...
    def __init__(self):
        # self.id = str(uuid4())
        self.id = 'Ginhu'
        self.blockchain = Blockcahain(self.id)

    def get_transaction_value(self):
        """Returns the input of the user (a new transaction amount)
        as a float."""

        # Get the user input, transform it from a string to a float
        # and store it in user_input
        ...
        while waiting_for_input:
            ...
            if user_choice == '1':
                ...
                if self.blockchain.add_transaction(
                  recipient, self.id, amount=amount):
